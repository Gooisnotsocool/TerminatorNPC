...
public class TerminatorTrait extends Trait {
	...
	// Built in stuck features only apply if the NPC is just far away it seems. The
	// name is misleading
	...
	public boolean canTarget(Player player) {
		return !(player.getGameMode().equals(GameMode.CREATIVE) || player.getGameMode().equals(GameMode.SPECTATOR)
				|| player.isInvulnerable() || TerminatorNPC.ignoredPlayers.contains(player.getUniqueId()));
	}
				...
				if (!delete) {
					try {
						if (npc.isSpawned()) {
								...
								if (canTarget(getTarget())) {
									if (blockPlaceCooldown == 0) {
										if (distanceToGround(4) && !isInWater) {
											Material p = Material.WATER;
											placeBlockUnderFeet(p);
										}
										if (getLivingEntity().getLocation().clone().subtract(0, 1, 0).getBlock()
												.getType().equals(Material.WATER) && !isFullSwimming) {
											placeBlockUnderFeet(terminatorLoadout.getBlockMaterial());
										}
									}
									if (getLivingEntity().getLocation().clone().subtract(0, 1, 0).getBlock().getType()
										...
										if (boatClutchCooldown == 0) {
											...
											getLivingEntity().getWorld().spawnEntity(
													getLivingEntity().getLocation().clone().subtract(0, 0.9, 0),
													EntityType.BOAT);
											tryJump(0.7, true);
							...
							if (getTarget() != null && getLivingEntity() != null
									&& getTarget().getWorld().getUID().equals(getLivingEntity().getWorld().getUID())) {
								if (isCurrentlyDiggingDownwards) {
									if (getLivingEntity().isOnGround()) {
										if (!getLivingEntity().getLocation().clone().subtract(0, 1, 0).getBlock()
												.getType().isAir()) {
											centerOnBlock();
											if (!scheduledBrokenBlocks.contains(getLivingEntity().getLocation().clone()
													.subtract(0, 1, 0).getBlock())) {
												if (debug) {
													Bukkit.getLogger().log(Level.INFO,
															"Added block underneath me to queued blocks. (Dig downwards)");
												}
												scheduledBrokenBlocks.add(getLivingEntity().getLocation().clone()
														.subtract(0, 1, 0).getBlock());
											}
										}
									}
									...
									if (blockPlaceCooldown == 0) {
										...
										if (getLivingEntity().isOnGround()) {
											if (aboveHeadIsAir()) {
												if (debug) {
													Bukkit.getLogger().log(Level.INFO,
															"Placed block underneath to go upwards.");
												}
												getLivingEntity()
														.teleport(getLivingEntity().getLocation().clone().add(0, 1, 0));
												lookDown();
												placeBlockUnderFeet(terminatorLoadout.getBlockMaterial());
											} else {
												if (debug) {
												...
												Block chosenBrokenBlock = getLivingEntity().getLocation().clone()
														.add(0, 2, 0).getBlock();
								...
								if (!isCurrentlyBreakingABlock && !isCurrentlyPlacingUpwards
										&& !isCurrentlyDiggingDownwards && !isBreakingWhileUpwards) {
									shouldJump = true;
								}
								update();
	...
	public void getProperToolToBreakBlock(Block chosenBrokenBlock) {
		if (chosenBrokenBlock.getType().equals(Material.DIRT)
				|| chosenBrokenBlock.getType().equals(Material.GRASS_BLOCK)
				|| chosenBrokenBlock.getType().equals(Material.GRAVEL)
				|| chosenBrokenBlock.getType().equals(Material.SAND)
				|| chosenBrokenBlock.getType().equals(Material.SOUL_SAND)
				|| chosenBrokenBlock.getType().equals(Material.SOUL_SOIL)) {
			setMainHandItem(new ItemStack(Material.DIAMOND_SHOVEL));
				...
				|| chosenBrokenBlock.getType().toString().toLowerCase().contains("wart")) {
			setMainHandItem(new ItemStack(Material.SHEARS));
		} else if (chosenBrokenBlock.getType().toString().toLowerCase().contains("log")
				|| chosenBrokenBlock.getType().toString().toLowerCase().contains("plank")
				|| chosenBrokenBlock.getType().toString().toLowerCase().contains("stem")) {
	...
	private boolean locationIsTeleportable(Location location) {
		if (location.clone().subtract(0, 1, 0).getBlock().getType().isSolid()
				&& !location.clone().subtract(0, 1, 0).getBlock().isLiquid() && location.getBlock().getType().isAir()) {
	...
	private void placeBlockUnderFeet(Material material) {
		...
		if (!getLivingEntity().isDead()) {
			if (canPlaceBlock(getLivingEntity().getLocation().clone().subtract(0, 1, 0))) {
				...
				getLivingEntity().getWorld().playSound(getLivingEntity().getLocation().clone().subtract(0, 1, 0),
						Sound.BLOCK_STONE_PLACE, 1, 1);
				blockPlaceCooldown = 10;
			} else {
				Bukkit.getServer().getScheduler().runTaskLater(TerminatorNPC.getPlugin(), new Runnable() {
					public void run() {
						placeBlockUnderFeet(material);
					}
				}, 8);
			}
		}
			...
			if (location.getBlock().getType().isAir() || !location.getBlock().getType().isSolid()
				...
				for (BlockFace blockFace : placeableBlockFaces) {
					if (location.getBlock().getRelative(blockFace).getType().isSolid()
							&& !location.getBlock().getRelative(blockFace).isLiquid()) {
						return true;
					}
				}
			}
	...
	private boolean isFarFromChosenBlock() {
		return !withinMargin(blockCurrentlyBeingBroken.getLocation(), getLivingEntity().getLocation(), 3)
				|| blockCurrentlyBeingBroken.getType().isAir();
	}
	...
	private void updateArmor() {
		...
		if (terminatorLoadout.getHelmetMaterial() != null) {
			...
			heartAdditions.add(((20
					* ArmorItemValues.valueOf(terminatorLoadout.getHelmetMaterial().toString()).getHealthMultiplier())
					- 20));
		}
		if (terminatorLoadout.getChestplateMaterial() != null) {
			...
			heartAdditions.add(((20 * ArmorItemValues.valueOf(terminatorLoadout.getChestplateMaterial().toString())
					.getHealthMultiplier()) - 20));
		}
		if (terminatorLoadout.getLeggingsMaterial() != null) {
			...
			heartAdditions.add(((20
					* ArmorItemValues.valueOf(terminatorLoadout.getLeggingsMaterial().toString()).getHealthMultiplier())
					- 20));
		}
		if (terminatorLoadout.getBootsMaterial() != null) {
			...
			heartAdditions.add(((20
					* ArmorItemValues.valueOf(terminatorLoadout.getBootsMaterial().toString()).getHealthMultiplier())
					- 20));
		}
				...
				if (!shouldBeStopped) {
					...
					if (block.isEmpty()) {
						...
						if (terminatorLoadout.getSwordMaterial() != null) {
							...
							AttackItemValues attackItemValues = AttackItemValues
									.valueOf(terminatorLoadout.getSwordMaterial().toString());
						...
						if (newHealth < health) {
							player.setVelocity(player.getVelocity()
									.add(getLivingEntity().getLocation().getDirection().multiply(0.3)));
						}
						PlayerAnimation.ARM_SWING.play((Player) getLivingEntity());
						scheduledBrokenBlocks.clear();
						...
						if (debug) {
							Bukkit.getLogger().log(Level.INFO,
									"Added block underneath me to queued blocks. (Block in way while attacking)");
						}
	...
	private void cooldownsDecrement() {
		...
		if (teleportTimer > 0) {
			teleportTimer--;
			if (teleportTimer == 0) {
				if (!locationIsVisible(getTarget(), getLivingEntity().getLocation())
						|| !withinMargin(getLivingEntity().getLocation(), getTarget().getLocation(), 60)) {
	...
	private void checkForLocationUpdate() {
		if (!updatedThisTick) {
			...
			if (withinMargin(location, getLivingEntity().getLocation(), 0.05) && !isInWater) {
				...
				if (!canSee(getTarget()) && !isCurrentlyBreakingABlock && !isCurrentlyPlacingUpwards
						&& !isCurrentlyDiggingDownwards) {
					if (debug) {
						Bukkit.getLogger().log(Level.INFO,
								"Added block infront of me to queued block breaks. (Mining towards target)");
					}
					if (!getBlockInFront(1).getBlock().isEmpty()
							&& !scheduledBrokenBlocks.contains(getBlockInFront(1).getBlock())) {
						scheduledBrokenBlocks.add(getBlockInFront(1).getBlock());
					}
					if (!getBlockInFront(1).subtract(0, 1, 0).getBlock().isEmpty() && !scheduledBrokenBlocks
							.contains(getBlockInFront(1).clone().subtract(0, 1, 0).getBlock())) {
	...
	private void enableMovement() {
		new BukkitRunnable() {
			@Override
			public void run() {
				if (!isCurrentlyBreakingABlock && !isBreakingWhileUpwards && !isCurrentlyPlacingUpwards
						&& !isCurrentlyDiggingDownwards) {
					shouldBeStopped = false;
					shouldJump = true;
					for (Trait trait : npc.getTraits()) {
