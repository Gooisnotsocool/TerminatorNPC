...
public class TerminatorCommands {
	@Command(aliases = {
			"terminator" }, usage = "loadout", desc = "View and modify the loadout for terminators", modifiers = {
					"loadout" }, min = 1, max = 1, permission = "terminatornpc.loadout")
	public void loadout(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		...
		TerminatorNPC.terminatorLoadoutHashMap.putIfAbsent(player.getUniqueId(),
				new me.swipez.terminatornpc.loadout.TerminatorLoadout(TerminatorNPC.getPlugin()));

		me.swipez.terminatornpc.loadout.TerminatorLoadout terminatorLoadout = TerminatorNPC.terminatorLoadoutHashMap
				.get(player.getUniqueId());
		terminatorLoadout.display(player);
	}

	@Command(aliases = {
			"terminator" }, usage = "ignore [player]", desc = "Make terminators ignore player", modifiers = {
					"ignore" }, min = 2, max = 2, permission = "terminatornpc.ignore")
	public void ignore(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		...
		if (TerminatorNPC.ignoredPlayers.contains(ignoredPlayer.getUniqueId())) {
			...
			sender.sendMessage(ChatColor.WHITE + "Terminators will no longer ignore " + ChatColor.RED
					+ ignoredPlayer.getDisplayName() + ChatColor.WHITE + ".");
			return;
		}

		TerminatorNPC.ignoredPlayers.add(ignoredPlayer.getUniqueId());
		sender.sendMessage(ChatColor.WHITE + "Terminators will now ignore " + ChatColor.RED
				+ ignoredPlayer.getDisplayName() + ChatColor.WHITE + ".");
	}

	@Command(aliases = {
			"terminator" }, usage = "add [playername] (count)", desc = "Summon terminator(s)", modifiers = {
					"add" }, min = 2, max = 3, permission = "terminatornpc.spawnterminator")
	public void add(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		...
		TerminatorSummoner summoner = new TerminatorSummoner(args.getString(1), (Player) sender,
				((Player) sender).getLocation(), amount);
		summoner.summon();
		sender.sendMessage(
				"Summoned terminator(s) with name " + ChatColor.RED + args.getString(1) + ChatColor.RESET + ".");
	}

	@Command(aliases = { "terminator" }, usage = "clear", desc = "Remove all terminators", modifiers = {
			"clear" }, min = 1, max = 1, permission = "terminatornpc.deleteterminator")
	public void clear(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		Player player = (Player) sender;
		player.sendMessage(
				"Clearing " + ChatColor.RED + TerminatorNPC.terminators.size() + ChatColor.RESET + " Terminators.");
	...
	@Command(aliases = { "terminator" }, usage = "count", desc = "Count all terminators", modifiers = {
			"count" }, min = 1, max = 1, permission = "terminatornpc.deleteterminator")
	public void count(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		Player player = (Player) sender;
		int count = 0;

		for (NPC terminator : TerminatorNPC.terminators) {
			if (terminator.isSpawned()) {
				count++;
			}
		}

		player.sendMessage(count + " Terminators left.");
	}

	@Command(aliases = {
			"terminator" }, usage = "summoner [playername] [seconds] (amount)", desc = "Create a terminator summoner at your location", modifiers = {
					"summoner" }, min = 3, max = 4, permission = "terminatornpc.spawnterminator")
	public void addSummoner(CommandContext args, CommandSender sender, NPC npc) throws CommandException {
		...
		int summonerId = Bukkit.getScheduler().scheduleSyncRepeatingTask(TerminatorNPC.getPlugin(), new Runnable() {
			...
			public void run() {
				...
				// Send a message to all players announcing that a new terminator has been
				// summoned
				Bukkit.getOnlinePlayers().forEach(p -> p.sendMessage(ChatColor.GOLD + "A new " + ChatColor.RESET
						+ summoner.getName() + ChatColor.GOLD + " has been spawned!"));
			}
		}, timer, timer);

		int id = TerminatorNPC.getUniqueID();
		sender.sendMessage("Created summoner with id " + ChatColor.RED + id + ChatColor.RESET + " at location "
	...
	@Command(aliases = {
			"terminator" }, usage = "removesummoner [id]", desc = "Remove a terminator summoner", modifiers = {
					"removesummoner" }, min = 2, max = 2, permission = "terminatornpc.spawnterminator")
